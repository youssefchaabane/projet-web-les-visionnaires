...
class RecetteController
{
    ...
    public static function obtenirTous()
    {
        try {
            ...
            $pdo = Config::getConnexion();
            $sql = "SELECT * FROM recette ORDER BY $sortBy $order LIMIT :limite OFFSET :offset";
    ...
    public static function rechercher()
    {
        try {
            ...
            $pdo = Config::getConnexion();
            $sql = "SELECT DISTINCT recette.* FROM recette 
    ...
    public static function obtenirStatistiques()
    {
        try {
            ...
            $top = [];
            $stmt2 = $pdo->query("SELECT nom, calories_totales FROM recette ORDER BY calories_totales DESC LIMIT 3");
    ...
    public static function exportPdf()
    {
        try {
            ...
            $title = $terme !== '' ? 'Recettes - Recherche : ' . $terme : 'Liste complète des recettes';
            $lines = [
                'Recettes exportées',
                'Date : ' . date('d/m/Y H:i'),
                str_repeat('-', 90)
            ...
            foreach ($recettes as $r) {
                ...
                $lines[] = 'Personnes : ' . $r['nombre_personnes'] . ' | Difficulté : ' . $r['difficulte'];
                $lines[] = 'Prépa : ' . $r['temps_preparation'] . ' min | Cuisson : ' . $r['temps_cuisson'] . ' min | Calories : ' . $r['calories_totales'];
                $lines[] = 'Ajoutée le : ' . date('d/m/Y', strtotime($r['date_creation']));
                $lines[] = str_repeat('-', 90);
            }
